...
import LearningAgentPage from "./LearningAgentPage"
    ...
    if (this.state.hasError) {
      return (
        <main className="dashboard-shell min-h-screen text-slate-100">
          <section className="mx-auto flex min-h-screen max-w-3xl items-center justify-center px-6 py-24">
            <div className="rounded-3xl border border-red-500/30 bg-red-950/90 p-8 text-center shadow-2xl">
              <div className="mb-4 text-sm font-semibold uppercase tracking-[.24em] text-red-400">Learning Agent unavailable</div>
              <h1 className="text-3xl font-black text-white">Something failed to load.</h1>
              <p className="mt-3 text-sm leading-6 text-slate-300">{this.state.message}</p>
              <button onClick={() => this.setState({ hasError: false, message: "" })} className="mt-6 rounded-full bg-cyan-400 px-5 py-3 text-sm font-bold text-slate-950">Try again</button>
            </div>
          </section>
        </main>
      )
...
export default function App() {
  ...
  if (screen === "dashboard") {
    return <Dashboard
      ...
      onLearningAgent={() => setScreen("learning-agent")}
      onReportsPage={() => setScreen("reports")}
      isBusy={isBusy}
    />
  }

  if (screen === "learning-agent") {
    return (
      <PageErrorBoundary>
        <LearningAgentPage onBack={() => setScreen("dashboard")} userProfile={userProfile} />
      </PageErrorBoundary>
    )
  }
  ...
  if (screen === "feedback" && feedback) {
    ...
    if (rt === "dsa") return <DSAFeedback report={r} onRestart={() => startRound("dsa")} onBack={() => setScreen("dashboard")} onLearn={() => setScreen("learning-agent")} />
    if (rt === "cs_fundamentals") return <CSFeedback report={r} onRestart={() => startRound("cs_fundamentals")} onBack={() => setScreen("dashboard")} onLearn={() => setScreen("learning-agent")} />
    if (rt === "project_behavioral") return <PBFeedback report={r} onRestart={() => startRound("project_behavioral")} onBack={() => setScreen("dashboard")} onLearn={() => setScreen("learning-agent")} />
    return <Feedback report={r} onRestart={() => startRound(rt)} onBack={() => setScreen("dashboard")} onLearn={() => setScreen("learning-agent")} />
  }

  if (screen === "learning-agent") {
    return <LearningAgentPage onBack={() => setScreen("dashboard")} userProfile={userProfile} />
  }
        ...
        <section className="relative z-10 mx-auto grid min-h-[calc(100vh-73px)] max-w-6xl content-center gap-6 px-6 py-10">
          <div className="interview-setup-card">
            ...
            {isProjectRound && <label className="mt-4 flex cursor-pointer items-center gap-3 rounded border border-dashed border-slate-700 p-4 text-sm text-slate-300">
              <Upload size={18} />
              <span>{resume ? resume.name : "Upload resume for personalized project and behavioral questions"}</span>
    ...
    <main className="dashboard-shell min-h-screen text-white">
      ...
      <section className="relative z-10 mx-auto grid min-h-screen max-w-7xl gap-10 px-6 py-10 lg:grid-cols-[minmax(0,.95fr)_minmax(360px,.65fr)] lg:items-center">
        <motion.div className="grid max-w-3xl gap-6" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
          ...
          <p className="max-w-2xl text-lg leading-8 text-slate-300">CodeVoir combines company-specific DSA practice, CS fundamentals, project deep-dives, behavioral probing, and live code execution so every attempt becomes a clearer next step.</p>
      ...
      <div className="rounded border border-slate-800 bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Traits to strengthen for project and behavioral rounds.</p>
          </div>
          <Brain className="text-cyan-300" size={20} />
        </div>
        {hasBehavior ? (
...
function Dashboard({ userProfile, dashboard, loading, error, onStart, onRefresh, onOpenReport, onLogout, onOpportunities, onLearningAgent, onReportsPage, isBusy }) {
  const stats = dashboard.stats || defaultDashboard.stats
  const interviews = dashboard.interviews || []
  const completed = interviews.filter((item) => item.has_report)
    ...
    <main className="dashboard-shell min-h-screen text-slate-100">
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl gap-5 px-6 pb-14 pt-20">
        <DashboardHero
          ...
          onLearningAgent={onLearningAgent}
          onRefresh={onRefresh}
          loading={loading}
        />
...
function DashboardHero({ displayName, stats, completed, onStart, onOpportunities, onLearningAgent }) {
  return (
    <motion.section className="dashboard-hero" initial="hidden" animate="show" variants={dashboardStagger}>
      <motion.div variants={dashboardCard} className="dashboard-hero-copy">
        ...
        <div className="dashboard-action-stack">
          ...
          <button onClick={onLearningAgent} className="dashboard-secondary-action"><Brain size={17} /> Learning Agent</button>
        </div>
...
function RoundShortcuts({ onStart }) {
  const rounds = [
    ...
    { label: "Project + behavioral", helper: "Resume evidence and STAR answers", tone: "purple", round: "project_behavioral" },
  ]
  return (
...
function Feedback({ report, onRestart, onBack, onLearn }) {
  ...
  return <main className="dashboard-shell min-h-screen p-6 text-slate-100">
    ...
    <section className="dashboard-glass relative z-10 mx-auto max-w-6xl p-6">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <button onClick={onRestart} className="rounded bg-cyan-400 px-4 py-2 font-semibold text-slate-950">Start DSA interview</button>
        {onLearn && <button onClick={onLearn} className="rounded border border-purple-400/50 bg-purple-500/10 px-4 py-2 font-semibold text-purple-100">Learn weak areas</button>}
      </div>
    </section>
  </main>
}
...
function DSAFeedback({ report, onRestart, onBack, onLearn }) {
    ...
    <main className="dashboard-shell min-h-screen text-slate-100">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <button onClick={onRestart} className="rounded bg-cyan-400 px-6 py-3 font-semibold text-slate-950 hover:bg-cyan-300 transition-colors">
            Start Another Interview
          </button>
          {onLearn && <button onClick={onLearn} className="rounded border border-purple-400/50 bg-purple-500/10 px-6 py-3 font-semibold text-purple-100 hover:border-purple-300">Learn weak areas</button>}
        </div>
      </div>
    </main>
  )
...
function CSFeedback({ report, onRestart, onBack, onLearn }) {
...
function PBFeedback({ report, onRestart, onBack, onLearn }) {
