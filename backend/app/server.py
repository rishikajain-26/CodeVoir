...
# Load backend/.env before reading provider, OAuth, and auth settings.
# This keeps local setup predictable when running with `uvicorn main:app`.
...
from app.learning_agent.routes import router as learning_agent_router
...
app = FastAPI(title="CodeVoir API", version=APP_VERSION, lifespan=_lifespan)
...
async def health_check():
    return {"status": "ok", "service": "codevoir", "version": APP_VERSION, "problems_count": len(DSA_PROBLEMS)}
...
def _frontend_url() -> str:
    return os.getenv("FRONTEND_URL", "http://127.0.0.1:5176").rstrip("/")
...
def _groq_interview_turn(session: dict[str, Any], user_text: str) -> str:
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            data=body,
            headers={"Content-Type": "application/json", "Authorization": f"Bearer {api_key}", "User-Agent": "CodeVoirInterviewLab/1.0"},
            method="POST",
        )
