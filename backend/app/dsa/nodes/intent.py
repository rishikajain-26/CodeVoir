...
def _fallback_turn_intent(state: DSAState) -> CandidateTurnIntentLLM:
    ...
    if any(phrase in text for phrase in ("can you see my code", "check my code", "go through my code", "review my code", "look at my code", "check out my code", "please go through", "see my code")):
        return CandidateTurnIntentLLM(intent="submitting_code", summary="Candidate is asking the interviewer to review their current code.")
...
def _apply_explicit_signal_overrides(state: DSAState, turn_intent: CandidateTurnIntentLLM) -> CandidateTurnIntentLLM:
    ...
    if any(phrase in text for phrase in _code_review_phrases):
        # Treat code-review requests as a normal submission intent, not a complaint.
        # The candidate is asking for feedback, not expressing frustration with the AI.
        return turn_intent.model_copy(
            update={
                "intent": "submitting_code",
                "summary": "Candidate is asking the interviewer to review their current code.",
                "frustration_level": int(turn_intent.frustration_level or 0),
            }
        )
    return turn_intent
...
def _to_candidate_intent(state: DSAState, turn_intent: CandidateTurnIntentLLM) -> CandidateIntent:
    ...
    should_end = any(
        ...
        for term in (
            "end interview", "end the interview", "stop interview",
            "stop the interview", "wrap up", "i'm done interviewing",
            "im done interviewing",
        )
    )
    should_advance = any(
        term in state.candidate_explanation.lower()
        for term in (
            "next question", "next problem", "move to the next",
            "done with this", "finished this problem",
            "skip this problem", "skip this question",
        )
    )
...
def _route_action_from_intent(state: DSAState, intent: CandidateIntent) -> str:
    ...
    # Hint takes priority over the frustration direct-reply path — a frustrated
    # but stuck candidate genuinely needs the hint, not an apology.
    if intent.should_give_hint and state.memory.hints_given < state.config.max_hints:
